...
/*On ajoute notre script JS pour le plugin Mondial Relay
function add_js_scripts() {
	wp_enqueue_script( 'script', get_template_directory_uri().'/js/script.js', array('jquery'), '1.0', true );
	wp_enqueue_script( 'script_facturation', get_template_directory_uri().'/js/script_facturation.js', array('jquery'), '1.0', true );
...
function ajout_ligne_mondialrelay() {
	...
	*/
	...
	foreach ( $mailClient as $go ) 
	{
		...
		if(creation_etiquette_mondialrelay($mon_numero_commande, $nomvendeur, $prenomclient, $nomclient, $pointrelais, $marue, $moncode, $maville, $MailVendeur, $MailClient, $mon_poids_commande, $ContenuColis)){
...
function creation_etiquette_mondialrelay($numcommande, $lenomvendeur, $leprenomclient,$lenomclient, $myshopnumber, $rueVendeur, $codepostalvendeur,$villevendeur, $monMailVendeur, $monMailClient, $monPoids, $monContenuColis){
	// On ajoute la lib de Mondial Relay
	require_once('inc/includesMondial/MondialRelay.API.Class.php');
	//On récupère le numéro de point relais FR-XXXXXXX et on enleve le "FR-"
	...
	//set the credentials
	$MRService->_Api_CustomerCode 	= "BDTEST13";//"CC21A7Z8"; BDTEST13
	$MRService->_Api_BrandId 		= "41";
	$MRService->_Api_SecretKey  	= "PrivateK";//"egtNTCJU"; PrivateK
	$MRService->_Api_User 	    	= "ced.bernard31@gmail.com";
	$MRService->_Api_Password 		= "Wfusdfcf1989360&";
	...
	$myShipment->Recipient->Adress2 = "dac";
	$myShipment->Recipient->Adress3 = "dac";
	$myShipment->Recipient->Adress4 = "dac";
	$myShipment->Recipient->PostCode = "06210";
	$myShipment->Recipient->City = "Bordeaux";
