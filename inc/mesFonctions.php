<?php
  //Fonction permettant de récupérer la range de poids max sur lequel se baser pour calculer le prix d'une expédition Mondial Relay
	...
	function send_email_facturation_expedition($to, $order_id, $num_mondial_relay, $prix_ttc, $nomBoutique, $laDate){
		//wp_mail($destinataire, $monSujet, $monMail);
		$order_id = $order_id;
		$mailByfrenchyz = "admin@byfrenchyz.fr";
		$subject = "Reçu Byfrenchyz expédition commande " . $order_id;
		//TVA 20%
		$tva = round($prix_ttc - $prix_ttc/1.2, 2);
		$prix_ht = $prix_ttc - $tva;
    ...
    <table style = 'margin-top : 40px;' class='email-wrapper' width='100%' cellpadding='0' cellspacing='0' role='presentation'>
      <tr>
		...
		wp_mail($to, $subject, $body, $headers );
		wp_mail($mailByfrenchyz, $subject, $body, $headers );
	}
	...
	function send_email_etiquette_mondial($numcommande, $numero_expedition, $lienPdf, $monMailClient, $monMailVendeur, $leprenomclient, $lenomclient, $lenomvendeur){
		$headers = array('Content-Type: text/html; charset=UTF-8');
		$mailByfrenchyz = "admin@byfrenchyz.fr";
		$lienTrouverPointRelais = "https://www.mondialrelay.fr/trouver-le-point-relais-le-plus-proche-de-chez-moi";

		///////////////////////////////////////////////////// Bloc d'envoi de mail admin ///////////////////////////////////////////////

		$SubjectMail = 'Nouvelle étiquette Mondial Relay pour la commande Byfrenchyz N° ' .$numcommande;

		$monBody = "<div>Bonjour,</div><br><br>
		<div>Veuillez trouver ci-dessous le numéro d'expédition Mondial Relay associée à la commande Byfrenchyz N°$numcommande effectuée ce jour : </div><br>
		<div>$numero_expedition</div><br>
		<div>Prénom Client: $leprenomclient</div><br>
		<div>Nom Client: $monMailClient</div><br>
		<div>Mail Client: $numero_expedition</div><br><br>

		<div>Nom Vendeur: $lenomvendeur</div><br>
		<div>Mail Vendeur: $monMailVendeur</div><br>
		<br><br>
		
		L'equipe Byfrenchyz";

		wp_mail($mailByfrenchyz, $SubjectMail, $monBody, $headers);
		...
		<div><a href=$lienTrouverPointRelais>Cliquez ici pour trouver un point de dépôt Mondial Relay proche de chez vous</a></div><br>
		...
		$SubjectMail = 'Numero de suivi de votre commande Byfrenchyz ' .$numcommande;

		$monBody = "<div>Bonjour $leprenomclient $lenomclient,</div><br><br>
		<div>Veuillez trouver ci-dessous votre numéro d'expédition Mondial Relay associée à votre commande Byfrenchyz N°$numcommande effectuée ce jour : </div><br>
		<div>$numero_expedition</div><br>
	...
	function send_email_facturation_commission($to, $order_id, $prix_ttc){
		//wp_mail($destinataire, $monSujet, $monMail);
		$order_id = $order_id;
		$mailByfrenchyz = "admin@byfrenchyz.fr";
		$subject = "Reçu commission Byfrenchyz commande " . $order_id;
		//TVA 20%
		$tva = round($prix_ttc - $prix_ttc/1.2, 2);
		$prix_ht = $prix_ttc - $tva;

		$body = "<div>Bonjour; <div> <br>
		<span class='preheader'>Veuillez trouver ci-dessous la facture correspondant à la commission perçue par Byfrenchyz sur le montant total de votre commande.</span>
		<br><br><div>Commande Byfrenchyz N° $order_id</div>
    <table style = 'margin-top : 40px;' class='email-wrapper' width='100%' cellpadding='0' cellspacing='0' role='presentation'>
      <tr>
        <td align='center'>
          <table class='email-content' width='100%' cellpadding='0' cellspacing='0' role='presentation'>
              ...
              <td class='email-body' width='100%' cellpadding='0' cellspacing='0'>
                <table class='email-body_inner' align='center' width='570' cellpadding='0' cellspacing='0' role='presentation'>
                  ...
                  <tr>
                    <td class='content-cell'>
                      <div class='f-fallback'>
                        <h1 align : center>Commission de vente Byfrenchyz.fr</h1>
                        <table style = 'margin-top : 20px;'class='attributes' width='100%' cellpadding='0' cellspacing='0' role='presentation'>
                          <tr>
                            <td class='attributes_content'>
                              <table width='100%' cellpadding='0' cellspacing='0' role='presentation'>
                                <tr>
                                  <td class='attributes_item'>
                                    <span class='f-fallback'>
									  <strong>Commission Byfrenchyz :</strong> $prix_ttc € TTC
									</span>
                                  </td>
                                </tr>
                              </table>
